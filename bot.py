...
def getResults():
    ...
    for i in range(0 , len(runs) // 2, 2):
        results.append('%s %s - %s %s' % (siglas[i].text, runs[i].text, siglas[i + 1].text, runs[i + 1].text))
...
def getTabla():
    ...
    for i in range(amount_teams):
        tabla.append('%s - %s - %s - %s' % (teams[i].text, data[(i + 1) * amount_teams].text, data[(i + 1) * amount_teams + 1].text, data[(i + 1) * amount_teams + 2].text))
